/**
 * OpenAPI generator for `@pops/media` — projects the ts-rest contract
 * in `src/contract/rest.ts` to a static `openapi/media.openapi.json`.
 *
 * The contract is the canonical declaration of the media wire surface;
...
import { mediaContract } from '../src/contract/rest.js';
...
function mediaSchemaTransformer({ schema }: { schema: unknown }): OpenApiSchema | null {
...
function main(): void {
  const document = generateOpenApi(
    mediaContract,
    {
      info: {
        title: '@pops/media',
        description:
          "OpenAPI projection of the media pillar's REST contract. " +
    ...
    {
      schemaTransformer: mediaSchemaTransformer,
  ...
  process.stdout.write(`[media] wrote OpenAPI projection to ${outFile}\n`);
}
