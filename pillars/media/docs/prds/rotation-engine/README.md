...
A daily automated cycle that keeps the movie library inside a free-disk-space budget. Each run: sweeps expired "leaving" movies (deleting them from Radarr), measures free space, marks the oldest eligible movies as "leaving" until the deficit is covered, then — if space permits — tops the library up with candidates from the queue (see [source-lists](../source-lists/README.md)). The count removed or added on any given day is variable, driven entirely by disk usage, never a fixed number.

The candidate queue, sources, and exclusion list live in **[source-lists](../source-lists/README.md)**; the rotation-log UI in **[rotation-ui](../rotation-ui/README.md)**. This PRD owns the cycle engine, the leaving state machine, the scheduler, and the rotation log.
...
Engine routes on the media pillar's ts-rest contract (`/rotation/...`). The data-plane queue routes (`/rotation/candidates`, `/rotation/exclusions`, `/rotation/sources`, `/rotation/source-types`, `/rotation/plex-friends`) belong to [source-lists](../source-lists/README.md).
