...
No tables of its own — writes the `tv_shows`, `seasons`, and `episodes` rows owned by the [data-model-api PRD](../data-model-api/README.md). Columns this flow populates:
...
The byte route `GET /media/images/:mediaType/:id/:filename` serves cached artwork directly from `MEDIA_IMAGES_DIR`. It is a plain Express route mounted alongside the contract and is deliberately **not** part of ts-rest (adds no OpenAPI paths) — see the [data-model-api PRD](../data-model-api/README.md).
...
- Movie metadata ([TMDB client PRD](../tmdb-client/README.md))
