...
- [x] `THETVDB_API_KEY` read from environment; startup validation fails with a clear error if missing — **deviation: validation is lazy init (returns null on missing key), not a hard startup failure; chosen for graceful degradation**
