...
1. **AI usage logging.** Every Claude API call from the worker reports usage, cost, latency, cache, and error telemetry to the **ai pillar**, which owns the cross-pillar `ai_inference_log` store. Food does not keep a local inference-log table or route; it reports through the shared `@pops/ai-telemetry` wrapper, tagging each record with `domain='food'` and operation strings specific to each ingest path. PRD-133 specifies how the worker reports and the record contents.

2. **Prompt viewer.** A read-only page at `/food/prompts` shows every prompt the food pipeline uses, with metadata (model, prompt version, description). Lets the user inspect and discuss prompts without opening the repo.

No food-local AI tables. Telemetry is owned by the ai pillar (theme 05 AI Operations); food is one of its reporting clients.

## AI Usage Logging

### Reporting API

Food reports telemetry through the shared `@pops/ai-telemetry` wrapper. There is no food-local logging helper, mutation, or route — the wrapper POSTs each record to the ai pillar's internal ingest endpoint.

```ts
// from @pops/ai-telemetry, used by every food worker Claude call site
...
`callWithLogging` runs the Claude call on the hot path and, fire-and-forget, looks up pricing, computes cost, and reports an `InferenceRecord` to the ai pillar's internal `POST /ai-usage/record` (gated by `x-pops-internal-token`). Each record carries `domain='food'`, the operation, the prompt version (merged into `metadata.prompt_version`), `context_id` (typically `ingest_source:<sourceId>`), and token/cost/latency/status. Sink target resolves from `AI_API_URL` (then `POPS_API_URL`) + `POPS_API_INTERNAL_TOKEN`; when neither URL resolves, reporting is a silent no-op. Telemetry never alters control flow.

### Record contents

For a food record, the ai pillar's `ai_inference_log` columns get:
...
`callWithLogging` from `@pops/ai-telemetry` wraps every Claude call so reporting is automatic. Each ingest handler (PRDs 127-132) uses it instead of calling the Anthropic SDK directly. The wrapper:

1. Records start time.
2. Awaits the Claude call.
3. On success: looks up pricing, computes cost, reports a `status='success'` record.
4. On error: reports a `status='error'` record (tokens 0) with the error message, then rethrows.

Reporting is fire-and-forget and never blocks or fails the call.

### Cost computation

The ai pillar's pricing is looked up per `(provider, model)` pair via `@pops/ai-telemetry`'s HTTP pricing adapter. If absent → cost is reported as 0 and flagged in metadata; surfaced in monthly AI ops review.

### Cost-cap observation

If a single Claude call exceeds the per-job cost cap (`FOOD_INGEST_COST_CAP_PER_JOB_USD`, default `0.05` USD; env var exposed in PRD-126's compose), the reported record carries `metadata.over_cost_cap=true` and the worker emits a console warning. v1 does NOT abort the call. Future enhancement: hard abort with `CostCapExceeded`.
...
- Reporting is **fire-and-forget from the worker's perspective** — failure to report does NOT fail the ingest. `@pops/ai-telemetry` swallows report errors (missing sink, non-2xx, thrown fetch) and continues. We don't want logging issues to block recipe ingestion.
- Every Claude call in PRDs 127-132 MUST go through the `callWithLogging` wrapper. Tests in each handler verify the wrapper was called.
...
| Telemetry report to the ai pillar fails (sink down or non-2xx)        | `@pops/ai-telemetry` swallows the error; worker continues. Ingest unaffected.                                                                 |
...
- [x] Every food worker Claude call goes through `@pops/ai-telemetry`'s `callWithLogging`; food keeps no local logging helper, mutation, or route.
- [x] Reports POST to the ai pillar's internal `POST /ai-usage/record`, auth'd via `x-pops-internal-token` (`POPS_API_INTERNAL_TOKEN`), sink resolved from `AI_API_URL` / `POPS_API_URL`.
- [x] Each handler in PRDs 127-132 uses `callWithLogging` for every Claude API call.
- [x] Vitest test: a mocked Claude call routed through the wrapper produces an `InferenceRecord` with all fields populated correctly.

### Cost computation

- [x] Wrapper looks up pricing per `(provider, model)`; computes cost from token counts.
- [x] Missing pricing → reports cost=0 with a `cost_missing` flag in metadata.
...
- [x] Vitest coverage of the `callWithLogging` reporting wrapper (`packages/ai-telemetry/src/__tests__/`) covers happy path, error path, missing-usage path, missing-pricing path.
