/**
 * Integration tests for the one-shot food → ai-pillar backfill. Uses a real
 * tmpdir-backed food SQLite (so the read path is exercised end-to-end) and a
 * fake `fetch` (the network boundary) to assert the script maps rows, POSTs
 * dedupe-keyed records with the internal token, and is safe to re-run.
 */
...
function seedRows(rows: Partial<AiInferenceLogRow>[]): void {
  const { db, raw } = openFoodDb(dbPath);
