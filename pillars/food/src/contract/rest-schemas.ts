 ...
 *
 * Split from `rest.ts` so the per-module route files (`rest-conversions.ts`,
 * `rest-ingredients.ts`, …) stay focused on their path maps. Everything
 ...
 * Used for the auto-increment ids (conversions, weights, …).
 */
export const PathPositiveInt = z.coerce.number().int().positive();

/** String identity (slug / asset-id style). Path + body alike. */
...
/**
 * Optional positive-int query param. Wire values are strings; coerce, but
 * keep it optional so an absent filter stays `undefined`.
 */
 ...
 * shared `HttpError` mapping (`mapHttpError`). `strictStatusCodes` is off
 * on the composer, but declaring them keeps the handler return types honest
 * and the OpenAPI projection complete.
 */
