...
- `finance.db` (separate SQLite file from `pops.db`, `core.db`, `inventory.db`, and `media.db`) is the canonical store for every finance-owned table: `entities`, `transactions`, `transaction_corrections`, `transaction_tag_rules`, `tag_vocabulary`, `budgets`, and `wish_list`. The Track N per-pillar cutovers (N1, N3, N4, N5) routed every consumer through `getFinanceDrizzle()`, and Theme 13 (5th pillar FULL EXIT) retired the boot-time `backfillFinanceFromShared` bridge once the last blockers — the `core.entities` handlers + search adapter — flipped via #3106. Every finance-owned table now writes directly to `finance.db`.
