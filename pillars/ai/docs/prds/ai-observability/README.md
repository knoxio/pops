...
- Every AI call — cloud, local, or cache hit — is logged via the [telemetry ingest](../ai-inference-monitoring/README.md). Cost is computed at write time from `ai_model_pricing`; local models record 0 cost but real tokens and latency.
...
- Proactive domain-level insights (spending anomalies — see [AI Inference & Monitoring](../ai-inference-monitoring/README.md)).
