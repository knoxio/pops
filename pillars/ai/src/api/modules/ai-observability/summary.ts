...
import { aiInferenceLog, settingsService, type AiDb } from '../../../db/index.js';
...
function fetchOverall(db: AiDb, windowStartIso: string, windowEndIso: string): OverallRow {
  const [row] = db
    .select({
...
function fetchByModel(db: AiDb, windowStartIso: string, windowEndIso: string): ModelBreakdown[] {
...
export function computeSummary(db: AiDb, opts: ComputeSummaryOptions = {}): ObservabilitySummary {
...
export function runSummary(db: AiDb, opts: RunSummaryOptions = {}): RunSummaryResult {
