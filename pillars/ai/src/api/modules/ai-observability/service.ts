...
import { aiInferenceLog, type AiDb } from '../../../db/index.js';
...
function fetchOverallStats(db: AiDb, where: ReturnType<typeof buildWhere>): OverallStats {
  const [row] = db
    .select({
...
export function getStats(db: AiDb, filters: ObservabilityFilters = {}): StatsOutput {
  const where = buildWhere(filters);
  const overall = fetchOverallStats(db, where);
  const groupings = buildGroupings(db, where, filters);
...
export function getLatencyStats(db: AiDb, filters: ObservabilityFilters = {}): LatencyStats {
  const where = buildWhere(filters);
...
export function getQualityMetrics(db: AiDb, filters: ObservabilityFilters = {}): QualityMetrics {
  const where = buildWhere(filters);
