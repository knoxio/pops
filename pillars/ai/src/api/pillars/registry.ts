...
export interface PillarRegistryOptions {
  /**
   * HTTP origin ai-api is reachable at. Required — `server.ts`
   * derives it from `AI_SELF_BASE_URL` (or falls back to
   * `http://localhost:PORT`) before passing it in. The registry
   * returns this as the synthetic `ai` entry's `baseUrl` after
...
export function getPillarRegistry(options: PillarRegistryOptions): readonly PillarRegistryEntry[] {
  ...
  const normalisedSelf = parseBareOrigin("ai's selfBaseUrl", options.selfBaseUrl);
  const withoutSelf = cached.filter((p) => p.id !== 'ai');
  return [{ id: 'ai', baseUrl: normalisedSelf }, ...withoutSelf];
}
