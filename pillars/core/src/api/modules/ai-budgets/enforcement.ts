...
function listApplicableBudgets(
  db: CoreDb,
  provider: string,
  operation: string
): ApplicableBudget[] {
...
function hasGlobalBudgetConflict(db: CoreDb): boolean {
 ...
 */
export function migrateLegacyBudgetSettings(db: CoreDb): void {
 ...
 * as block.
 */
export function findFallbackProvider(db: CoreDb): { provider: string; model: string } | null {
  const row = db
    .select({
