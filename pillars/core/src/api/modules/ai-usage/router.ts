/**
 * tRPC router for AI usage analytics (`core.aiUsage.*`).
 *
 * Procedures:
...
export const aiUsageRouter = router({
  ...
  getStats: protectedProcedure.query(({ ctx }) => {
    return getStats(ctx.coreDb);
  }),

  /**
   * Get AI usage history filtered by date range
   */
  getHistory: protectedProcedure.input(getHistoryInputSchema).query(({ input, ctx }) => {
    return getHistory(ctx.coreDb, input.startDate, input.endDate);
  }),
