...
function fetchOverallStats(db: CoreDb, where: ReturnType<typeof buildWhere>): OverallStats {
  const [row] = db
    .select({
...
export function getStats(db: CoreDb, filters: ObservabilityFilters = {}): StatsOutput {
...
export function getLatencyStats(db: CoreDb, filters: ObservabilityFilters = {}): LatencyStats {
  const where = buildWhere(filters);
...
export function getQualityMetrics(db: CoreDb, filters: ObservabilityFilters = {}): QualityMetrics {
  const where = buildWhere(filters);
