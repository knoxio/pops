...
import { aiInferenceLog, settingsService, type CoreDb } from '../../../db/index.js';
...
function fetchOverall(db: CoreDb, windowStartIso: string, windowEndIso: string): OverallRow {
  const [row] = db
    .select({
...
function fetchByModel(db: CoreDb, windowStartIso: string, windowEndIso: string): ModelBreakdown[] {
...
export function computeSummary(db: CoreDb, opts: ComputeSummaryOptions = {}): ObservabilitySummary {
  ...
  const overall = fetchOverall(db, windowStart, windowEnd);
  const byProvider = fetchByProvider(db, windowStart, windowEnd);
  const byModel = fetchByModel(db, windowStart, windowEnd);
...
export type RunSummaryOptions = ComputeSummaryOptions;

export interface RunSummaryResult {
  summary: ObservabilitySummaryEnvelope;
}

export function runSummary(db: CoreDb, opts: RunSummaryOptions = {}): RunSummaryResult {
  const summary = computeSummary(db, opts);
  const computedAt = summary.windowEnd;
  const envelope: ObservabilitySummaryEnvelope = { ...summary, computedAt };

  settingsService.setRawSetting(db, OBSERVABILITY_SUMMARY_SETTING_KEY, JSON.stringify(envelope));
