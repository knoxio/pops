...
export function listRules(db: CoreDb): AlertRule[] {
  return db.select().from(aiAlertRules).all().map(ruleRowToRule);
}

export function getRule(db: CoreDb, id: number): AlertRule | null {
  const [row] = db.select().from(aiAlertRules).where(eq(aiAlertRules.id, id)).all();
  return row ? ruleRowToRule(row) : null;
}

export function createRule(db: CoreDb, input: CreateAlertRuleInput): AlertRule {
  const now = nowIso();
  const result = db
    .insert(aiAlertRules)
    .values({
 ...
 */
export function updateRule(db: CoreDb, input: UpdateAlertRuleInput): AlertRule {
  const now = nowIso();
...
export function setRuleEnabled(db: CoreDb, id: number, enabled: boolean): AlertRule {
  return updateRule(db, { id, enabled });
}

export function deleteRule(db: CoreDb, id: number): { success: boolean } {
  const result = db.delete(aiAlertRules).where(eq(aiAlertRules.id, id)).run();
 ...
 */
export function seedDefaultRules(db: CoreDb): number {
  ...
  const existingTypes = new Set(listRules(db).map((r) => r.type));
