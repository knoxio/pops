 ...
 *
 *   rules.list / rules.get / rules.create / rules.update / rules.delete /
 *   rules.setEnabled / rules.seedDefaults
...
export const aiAlertsRouter = router({
  rules: router({
    list: protectedProcedure.query(({ ctx }) => service.listRules(ctx.coreDb)),
    get: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .query(({ input, ctx }) => {
        const rule = service.getRule(ctx.coreDb, input.id);
        if (!rule) throw new TRPCError({ code: 'NOT_FOUND', message: 'Alert rule not found' });
        return rule;
      }),
    create: protectedProcedure
      .input(createInput)
      .mutation(({ input, ctx }) => service.createRule(ctx.coreDb, input)),
    update: protectedProcedure
      .input(updateInput)
      .mutation(({ input, ctx }) => service.updateRule(ctx.coreDb, input)),
    setEnabled: protectedProcedure
      .input(z.object({ id: z.number().int().positive(), enabled: z.boolean() }))
      .mutation(({ input, ctx }) => service.setRuleEnabled(ctx.coreDb, input.id, input.enabled)),
    delete: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ input, ctx }) => service.deleteRule(ctx.coreDb, input.id)),
    seedDefaults: protectedProcedure.mutation(({ ctx }) => ({
      created: service.seedDefaultRules(ctx.coreDb),
    })),
  }),
  list: protectedProcedure
    .input(listAlertsInput)
    .query(({ input, ctx }) => listAlerts(ctx.coreDb, input ?? {})),
  acknowledge: protectedProcedure
    .input(z.object({ id: z.number().int().positive() }))
    .mutation(({ input, ctx }) => {
      const alert = acknowledgeAlert(ctx.coreDb, input.id);
      if (!alert) throw new TRPCError({ code: 'NOT_FOUND', message: 'Alert not found' });
      return alert;
    }),
  runNow: protectedProcedure.mutation(({ ctx }) => runEvaluation(ctx.coreDb)),
});
