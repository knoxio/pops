/**
 * OpenAPI generator for `@pops/core` — projects the ts-rest contract in
 * `src/contract/rest.ts` to a static `openapi/core.openapi.json`.
 *
 * The contract is the canonical declaration of the core REST wire surface;
...
function coreSchemaTransformer({ schema }: { schema: unknown }): OpenApiSchema | null {
...
function main(): void {
  const document = generateOpenApi(
    coreContract,
    {
      info: {
        title: '@pops/core',
        description:
          "OpenAPI projection of the core pillar's REST contract. " +
    ...
    {
      schemaTransformer: coreSchemaTransformer,
  ...
  process.stdout.write(`[core] wrote OpenAPI projection to ${outFile}\n`);
}
