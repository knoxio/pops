...
Public contract for the **lists** pillar — types, Zod schemas, tRPC router
...
src/
├── index.ts        barrel
├── types/          plain TypeScript entity types (ListItem, …)
├── schemas/        Zod runtime validators matching ./types 1:1
├── router.ts       tRPC router *type* extracted from pops-lists-api (no runtime)
├── errors.ts       discriminated error envelope { kind: 'ok' | 'not-found' | … }
├── manifest.ts     <ListsContract> structural snapshot for the registry
└── __tests__/      round-trip Zod ↔ TS tests
openapi/
└── lists.openapi.json   generated; committed for iOS Swift codegen
scripts/
...
The current snapshot ships a single `ListItem` entity as a deliberate stub —
...
`completed`, `lastEditedTime`) and does **not** mirror the live lists domain
types; the full surface migration is follow-up work (PRD-153 US-07-style
content migration for lists).

## Rules

- **Zero runtime deps on `@pops/lists-db`.** `lists-db` is not a
  dependency of this package; the router type is extracted from
  `@pops/lists-api` (dev-only, when wired). Consumers depend on this
  package.
...
- **OpenAPI regenerates on every build.** `scripts/generate-openapi.ts`
  runs as part of `pnpm build`. The committed `openapi/lists.openapi.json`
