...
/**
 * Minimal structural contract for a scope-inference engine. The REST surface
 * does not wire one (the contract requires explicit scopes or a template that
 * supplies `default_scopes`), but the create path keeps the seam so a future
 * rule engine can be injected without a signature change.
 */
export interface ScopeInferenceEngine {
  inferScopes(input: {
    source?: string;
    type?: string;
    tags?: string[];
