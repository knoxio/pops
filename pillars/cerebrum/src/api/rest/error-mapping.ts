/**
 * Map cerebrum service errors to ts-rest response envelopes.
 *
 * Every domain service throws `HttpError` subclasses carrying a real
 * `statusCode` (`NotFoundError` → 404, `ConflictError` → 409,
 * `ValidationError` → 400). For those mapped statuses we return a typed
 * `{ status, body }` envelope; anything else (a 500-class `HttpError`, or a
