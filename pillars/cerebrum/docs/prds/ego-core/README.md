...
- **Cross-pillar action tools**: AI-callable actions across modules are not a verb registry baked into Ego — each pillar declares its tools in the `ai.tools` manifest dimension and the orchestrator projects the live, health-filtered set at `GET /ai/tools` ([AI-tool registry](../../../../orchestrator/docs/prds/ai-tool-registry/README.md)). When the model elects to call one, the qualified `<pillar>.<tool>` invocation routes back to its owning pillar over the SDK and the result is persisted in `messages.tool_calls`. A new AI-callable pillar surfaces to Ego with no cerebrum edit. Tool execution is always user-prompted within a turn — autonomous/reflex actions remain out of scope (Reflex owns those).
