...
function cerebrumSchemaTransformer({ schema }: { schema: unknown }): OpenApiSchema | null {
 ...
 * consumers (the defs live under a response schema, not the document root).
 * Hoist every nested `definitions` / `$defs` entry into the document-level
...
function main(): void {
  const document = generateOpenApi(
    ...
    {
      schemaTransformer: cerebrumSchemaTransformer,
  ...
  process.stdout.write(`[cerebrum] wrote OpenAPI projection to ${outFile}\n`);
}
