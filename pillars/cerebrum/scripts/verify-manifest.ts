 ...
 * instruction. Wired into the package's `build` script so a stale committed
 * manifest fails CI.
 */
...
try {
  ...
  console.error(
    `[cerebrum] ${MANIFEST_OUTPUT_PATH} is missing. Run \`pnpm -F @pops/cerebrum generate:manifest\` and commit the result.`
  );
  process.exit(1);
}
...
if (actual !== expected) {
  console.error(
    `[cerebrum] ${MANIFEST_OUTPUT_PATH} is out of date. Run \`pnpm -F @pops/cerebrum generate:manifest\` and commit the result.`
  );
  process.exit(1);
}

process.stdout.write(`[cerebrum] manifest.generated.ts is up to date (version=${version})\n`);
