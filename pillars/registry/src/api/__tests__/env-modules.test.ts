...
describe('PRD-100 env-modules (pillar copy)', () => {
  ...
  it('parses a single app and trims whitespace', () => {
    ...
    expect(readInstalledModules().apps).toEqual(['finance']);
  });

  it('parses a comma-separated list and deduplicates', () => {
    process.env[APP_KEY] = 'finance,inventory,finance';
    expect(readInstalledModules().apps).toEqual(['finance', 'inventory']);
  });

  it('throws on unknown app id, naming the bad value', () => {
  ...
  it('preserves operator-specified app ordering', () => {
    ...
    expect(readInstalledModules().apps).toEqual(['media', 'finance', 'cerebrum']);
  });
