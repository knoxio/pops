/**
 * Map inventory service errors to ts-rest response envelopes.
 *
 * Every module service throws `HttpError` subclasses carrying a real
 * `statusCode` (`NotFoundError` → 404, `ConflictError` → 409,
 * `ValidationError` → 400). For those four mapped statuses we return a
 * typed `{ status, body }` envelope; anything else (a 500-class
 * `HttpError`, or a non-HttpError) is re-thrown so Express's error
...
/**
 * Run a handler body and convert any mapped `HttpError` into its
 * response envelope. Accepts sync or async bodies. Unmapped throws
