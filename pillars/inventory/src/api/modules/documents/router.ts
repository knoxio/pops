...
export const documentsRouter = router({
  ...
  link: protectedProcedure.input(LinkDocumentSchema).mutation(({ input, ctx }) => {
    try {
      const row = service.linkDocument(ctx.inventoryDb, {
        itemId: input.itemId,
  ...
  unlink: protectedProcedure
    ...
    .mutation(({ input, ctx }) => {
      try {
        service.unlinkDocument(ctx.inventoryDb, input.id);
  ...
  listForItem: protectedProcedure.input(DocumentQuerySchema).query(({ input, ctx }) => {
    const limit = input.limit ?? DEFAULT_LIMIT;
    const offset = input.offset ?? DEFAULT_OFFSET;

    const { rows, total } = service.listDocumentsForItem(
      ctx.inventoryDb,
      input.itemId,
      limit,
      offset
