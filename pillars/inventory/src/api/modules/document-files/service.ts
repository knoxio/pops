...
function getUploadedFile(db: InventoryDb, id: number): ItemUploadedFileRow {
...
export function uploadDocument(db: InventoryDb, input: UploadDocumentInput): ItemUploadedFileRow {
  ...
  return getUploadedFile(db, id);
}

/** Remove an uploaded file by ID. Deletes both the DB record and the disk file. */
export function removeUpload(db: InventoryDb, id: number): void {
