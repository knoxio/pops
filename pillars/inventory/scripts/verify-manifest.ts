...
try {
  ...
  console.error(
    `[inventory] ${MANIFEST_OUTPUT_PATH} is missing. Run \`pnpm -F @pops/inventory generate:manifest\` and commit the result.`
  );
  process.exit(1);
}
...
if (actual !== expected) {
  console.error(
    `[inventory] ${MANIFEST_OUTPUT_PATH} is out of date. Run \`pnpm -F @pops/inventory generate:manifest\` and commit the result.`
  );
  process.exit(1);
}

process.stdout.write(`[inventory] manifest.generated.ts is up to date (version=${version})\n`);
