...
| --- | -------------------------------------------------------------------------------------------------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | **Finance reclaim** — `corrections`, `tag-rules`, `entities`↔`transactions` join live in `pillars/finance`, not core | **Done**                 | Finance owns the full corrections surface — CRUD + ChangeSet (#3449) + AI cluster (#3451) — the `entities`↔`transactions` usage rollup (#3450), and the FE consumes it over REST (#3452, all 13 call-sites). The monolith copies (`apps/pops-api/src/modules/core/{corrections,tag-rules,entities}`) are now dead-by-design; their deletion is an `02` task |
