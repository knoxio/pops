...
DONE:  ✅ 04 Phase 0 (#3444) · Track B (#3446) · Track C (#3447) · 01 core pillar (#3448) · C1 finance reclaim (#3449–#3452)
  ...
  └──────────────────────────────────────────────────────────────────────────┘
        │
        ▼
  ☐ RESIDUALS — G1's letter ("no usePillar* hooks") not yet met:
     1. pops-shell still uses REST-backed usePillar* (8 files: features/settings) — DECISION:
        convert to core-api Hey client (goal-consistent w/ the 8 apps) OR accept as generic SDK.
        + rename the now-misnamed useTrpcOptionsLoaders.ts
     2. 6 e2e specs still mock /trpc/transactions.* — rewire to REST mocks (test debt)
     3. stale comments (scripts/audit, core settings JSDoc) — cosmetic
