...
- Endpoints exposed: `GET /health`, and the tRPC surface at `/trpc` (currently hosting `media.shelfImpressions.*` after Track M3 PR 1; this is a shadow surface until the PR 2 cutover — pops-api still owns the canonical routing today). The tRPC surface honours the same `X-API-Key` / Cloudflare-JWT auth contract as `pops-api`.
