...
Every `UriResolverResult.kind` is produced by one or more of the sites listed in the table below and accepted by the `core.uri.resolve` Zod output schema in `apps/pops-api/src/modules/core/uri/router.ts`. Verified by inspection:
...
| ----------- | ---------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core`      | no                     | —                                 | `core` owns no cross-referenceable objects today. The dispatcher reserves `pops:core/...` for future probes (the integration smoke feeds `pops:core/probe/none` into `core.uri.resolve` to exercise the resolver end-to-end; the harness checks for "no such table" failures rather than asserting on the returned `kind`). Adding a handler is a no-op until `core` exposes addressable rows.                                                                                                              |
...
| `inventory` | yes                    | `item`, `location`                | Search adapter for inventory items emits a frontend route (`/inventory/items/<id>`), not a `pops:` URI. Inconsistency: `SearchHit.uri` is contractually a `pops:` URI (see `packages/types/src/search.ts`) and `packages/navigation/src/uri-resolver.ts` returns `null` for anything that doesn't start with `pops:`, so these hits won't navigate via `useSearchResultNavigation()`. The adapter should emit `pops:inventory/item/<id>` and let the frontend route map resolve it; tracked as a follow-up. |
