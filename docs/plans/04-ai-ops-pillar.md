...
- **Food reports cross-pillar (no local table/route).** Food's local `ai_inference_log` table, `POST /ai/log-inference` route (`foodAiContract.logInference`), and the `ai-handlers.ts` writer were removed (#3490). Food worker Claude calls now report through `@pops/ai-telemetry`'s `callWithLogging` to the ai pillar's internal `POST /ai-usage/record`, tagging `domain='food'` and merging `prompt_version` into metadata, best-effort.
- **The wrapper is shared, not food-local.** The React-coupled `pillars/food/app/src/ai/log-inference*.ts` files and the duplicated backend copy were deleted; the canonical wrapper lives in `@pops/ai-telemetry` (`callWithLogging`/`callWithLoggingStream`/`reportInference`), with the env POST sink resolving `AI_API_URL` (then `POPS_API_URL`) + `POPS_API_INTERNAL_TOKEN` and no-opping when unset.
