...
| **ai-ops** (AI pillar extraction + telemetry)      | `@pops/ai-telemetry` (TS wrapper, req+stream); `crates/pops-ai` (Rust crate, FIRST crate); `POST /ai-usage/record` wire; `GET /ai-pricing/:p/:m`; the `ai` pillar (port 3008)                                                             | `@pops/pillar-settings` (soft — interim local settings table until it lands); the `crates/` workspace (single-owner negotiation w/ contacts)                                    |
...
| **registry-rename** (capstone — separate artifact) | `pillars/core`→`pillars/registry`, image/DNS/env/nginx/CI/litestream/docs rename + DNS dual-alias window                                                                                                                                  | ALL THREE extractions complete (entities, ai-\*, settings left core) AND registry-cleanup's path rename settled                                                                 |
...
```
  ...
  contacts.Phase0 ─────────────► crates/Cargo.toml  (WORKSPACE ROOT, single owner)
   (or ai-ops.N8, whichever          │        │
    lands first — §3 negotiation)    │        │
                                     ▼        ▼
...
Both `contacts` (Phase 0) and `ai-ops` (N8) need `crates/Cargo.toml` + a Rust CI/mise lane. **Designated rule: whichever plan's Rust scaffolding lands FIRST authors `crates/Cargo.toml` + the `ai:rust`/Rust CI lane; the other ONLY adds its crate to `[workspace].members` and reuses the lane.** Default owner = **ai-ops** (it ships the first crate, `pops-ai`, and its `ai:rust` mise task is the lane). If contacts' Phase 0 lands first, contacts owns it and ai-ops adds `pops-ai` as a member. Exactly one plan authors the root; the program tracker (this doc) records who got there first.
...
TIER 0 — workspace + wrappers (no pillar consumes yet)
  [0a] crates/Cargo.toml workspace root           (ai-ops OR contacts, single owner §1.3)
...
| **P-D0** contacts Phase 0 (crate scaffold + closed-set Record updates + `crates/` root if first) | D     | A0, B0, C0               | — (but coordinates `crates/` ownership §1.3)                     |
...
- **Day 0 fan-out (4 lanes):** A0 (registry paths), B0 (`@pops/ai-telemetry`), C0 (`@pops/pillar-settings`), D0 (contacts scaffold). Zero cross-deps; one shared touch is the `crates/Cargo.toml` root (negotiated §1.3) and the closed-set Record updates (`PILLARS`, `ALL_MODULE_IDS`, `PILLAR_UPSTREAMS`) — coordinate so `contacts` and `ai` are added to those maps in one merge to avoid typecheck conflicts (both touch `known-pillar-id.ts`, `module-id.ts`, `generate-nginx-conf.ts`, `known-modules.ts`).
...
STAGE 1  — Shared foundations (parallel)
  ...
  1d contacts N0 scaffold + closed-set Records ─ GATE-G0: pnpm -w typecheck; registry:build clean; cargo build/test -p contacts
       [crates/ root authored here OR at 1b — single owner §1.3]
...
STAGE 5  — CAPSTONE (separate artifact, runs ONLY after STAGE 4 extractions are done)
  5  core→registry rename ─ see renamePlanMarkdown; its own GATEs RN0–RN6 + DNS dual-alias DEPLOY-OBSERVE window
```
...
| R-04 | Crates-workspace double-authorship (both plans create `crates/Cargo.toml`)                                                                                                 | contacts + ai-ops                                      | High         | Single designated owner (§1.3); other adds to `[workspace].members` only                                                                                                                | G0 / GATE-AI-P5                                 |
...
5. **STAGE 5** is the capstone rename — fired ONLY once core is "essentially just the registry" and the path rename has settled. Its full plan, tests, and rolling-deploy compat are in `renamePlanMarkdown`.
