...
| 05  | [us-05-media-call-site-burn-down](us-05-media-call-site-burn-down.md) — **Partial** | Flip the 4 media-side files to `pillar('cerebrum').debrief.*`. Implement the Option D mixed-tx pattern in `log-watch-event.ts`. Rewrite `getDebriefByMedia` to use `getByMedia` (no SQL inner-join). Drop the cross-pillar imports + matching allow-list entries. **US-05c done**: `watch-history/handlers/log-watch-event.ts` + `query-helpers.ts` flipped via `cerebrum-fan-out.ts` (Option D, fire-and-forget on the write path; awaited cascade on the delete path). Remaining sites under `comparisons/lib/*` and `media/debrief/{service,queue-status}.ts` belong to sibling slices. | Blocked by US-02 + US-03 + US-04           |
