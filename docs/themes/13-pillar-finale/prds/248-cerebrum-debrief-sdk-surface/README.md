...
>
> Status: **Partial — surface shipped, consumers removed pending wider SDK.** US-01..US-04 landed the SDK surface on `cerebrum-api`. US-05 stalled five times on the SDK ↔ consumer shape mismatch; rather than block the pillar migration further, the in-monolith debrief consumers were ripped out in `chore/remove-debrief-feature`. SDK procedures + `cerebrum-db` `debrief*` tables retained for future restoration. See [notes/debrief-feature-removal-2026-06.md](../../notes/debrief-feature-removal-2026-06.md).
...
try {
  ...
  if (err instanceof PillarCallError) {
    logger.warn(
      { err, watchHistoryId },
      'cerebrum debrief queue deferred — will self-heal on next watch or reconciler'
    );
...
| 02  | [us-02-write-surface](us-02-write-surface.md) — **Done**                           | Mount the write procedures on `cerebrumRouter`: `record`, `create`, `logWatchCompletion`. Each wraps its writes in a cerebrum-side transaction. `logWatchCompletion` is the Option D entry point.                                                               | Blocked by US-01; can split per-procedure  |
...
| 04  | [us-04-delete-and-dismiss-surface](us-04-delete-and-dismiss-surface.md) — **Done** | Mount `deleteByWatchHistoryId` + `dismiss`. Cascade-delete semantics for the former; idempotent dismiss for the latter.                                                                                                                                         | Blocked by US-01; parallel with US-02 / 03 |
| 05  | [us-05-media-call-site-burn-down](us-05-media-call-site-burn-down.md)              | **Deferred** — stalled 5x on SDK shape mismatch. Consumers removed in `chore/remove-debrief-feature`. See [notes/debrief-feature-removal-2026-06.md](../../notes/debrief-feature-removal-2026-06.md).                                                           | Deferred                                   |
| 06  | [us-06-integration-test](us-06-integration-test.md)                                | **Deferred** — depends on US-05 consumers. See [notes/debrief-feature-removal-2026-06.md](../../notes/debrief-feature-removal-2026-06.md).                                                                                                                      | Deferred                                   |
