...
| ---------------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `inventory.paperless.status` | query | Returns `{ data: { configured: boolean, available: boolean, baseUrl: string \| null } }`. `baseUrl` is `null` when the integration is disabled (`PAPERLESS_ENABLED=false`) or env vars (`PAPERLESS_URL` / `PAPERLESS_TOKEN`) are missing — in that case `configured` and `available` are both `false`. Reachability is probed by hitting `/api/document_types/`. |
