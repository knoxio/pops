...
>
> Status: **In progress** — US-01 core-api router mounted; in-monolith rename + ensure-expose, US-02 pattern doc, US-03 burn-down, US-04 integration test outstanding. Direct unblock for [PRD-246](../246-shell-api-pillar-decoupling/README.md) US-04 Site 8 (15 media files reading `@pops/core-db` directly).
...
| 03  | [us-03-media-call-site-burn-down](us-03-media-call-site-burn-down.md)     | Flip the 15 media files under `arr/`, `plex/`, `rotation/` from `settingsService.*` to `await pillar('core').settings.*`. Hot Plex paths use `getMany`. Allow-list entries removed.                                                                                    | Not started | Blocked by US-01; can split into per-dir PRs   |
| 04  | [us-04-integration-test](us-04-integration-test.md)                       | End-to-end test that starts `pops-core-api` + `pops-api`, has pops-api call `pillar('core').settings.{get,set,getMany}` from a media handler, and verifies wire-level correctness + the discovery cache.                                                               | Not started | Blocked by US-01                               |
