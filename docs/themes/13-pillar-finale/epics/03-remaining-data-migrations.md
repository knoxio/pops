...
| 245     | [`@pops/db-types` decomposition + cross-pillar FK drop](../prds/245-db-types-decomposition/README.md) | Relocate the 138 drizzle table files in `packages/db-types/src/schema/` into their owning `-db` packages; drop the 4 cross-pillar FK pairs (audit H6 + H7) | Done        |
