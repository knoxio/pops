...
| 244     | [`@pops/db-types` decomposition + cross-pillar FK drop](../prds/244-db-types-decomposition/README.md) | Relocate the 138 drizzle table files in `packages/db-types/src/schema/` into their owning `-db` packages; drop the 4 cross-pillar FK pairs (audit H6 + H7) | Not started |

Slices are mostly independent — parallel agent fleet shipped Theme 12's 6 finance slices in one night; same pattern works here. Cross-slice ordering only matters where FK relationships span slices (e.g. cerebrum.engrams may reference media URIs). PRD-244 is the structural cleanup that closes audit #3215 H6/H7 and is a **serial** sequence (8 USs that all touch `db-types/src/schema/index.ts` — same collision shape as PRD-239).
