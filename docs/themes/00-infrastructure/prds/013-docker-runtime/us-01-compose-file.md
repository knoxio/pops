...
- [x] `infra/docker-compose.yml` defines all services (pops-api, pops-shell, metabase, cloudflared, moltbot, paperless-ngx, paperless-redis)
- [x] Three networks defined: pops-frontend, pops-backend, pops-documents
- [x] Each service assigned to correct network(s)
- [x] Named volumes for persistent data (database, paperless, poster cache)
- [x] `depends_on` with health check conditions for startup ordering
