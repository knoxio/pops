...
Rename `apps/pops-api/` to `apps/pops-api/`. Restructure modules into domain groupings. Promote entities to `core/`. Establish the pattern for adding new domain modules.

## Scope

### In scope

- Rename `apps/pops-api/` → `apps/pops-api/`, `@pops/api` → `@pops/api`
...
1. `apps/pops-api/` exists, `apps/pops-api/` is deleted
...
- **Docker image name changes** — `pops-api` image name used in docker-compose and Ansible. All references need updating.
