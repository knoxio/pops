...
Persistence (the `pillar_registry` table, register/heartbeat/snapshot routes) belongs to [registry schema & endpoints](../../../federation/prds/registry-schema-endpoints/README.md). The SSE transport that carries change events is the [subscription model](../subscription-model/README.md). Boot reconciliation that seeds `unknown` is [reconciliation on restart](../../../federation/prds/reconciliation-on-restart/README.md). This PRD is the status engine that sits between them.
...
| `unknown`     | registry restarted; the pillar missed its threshold during the outage and has not re-heartbeated yet (seeded by [reconciliation on restart](../../../federation/prds/reconciliation-on-restart/README.md) boot reconciliation) |
...
This PRD adds **no new endpoints**. It layers behaviour onto the [registry schema & endpoints](../../../federation/prds/registry-schema-endpoints/README.md) routes plus a background process:
