...
The `@pops/pillar-sdk/capabilities` module ships the type machinery that _could_ bind a `<Pillar>Contract` to a typed proxy (`CallablePillar<C>`, `InputOf`/`OutputOf`, list projections — see [Capability projection types](../capability-projection-types/README.md)), but it is **not** wired into the runtime `pillar()`. Connecting them — the `declareContracts`/`ContractFor<P>` mechanism the original spec proposed — is unbuilt; see [idea](../../../../ideas/type-generation-pipeline.md) and the existing [client-surface](../../../../ideas/client-surface.md) and [capability-projection-types](../../../../ideas/capability-projection-types.md) ideas.
