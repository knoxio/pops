...
The complementary half — building the list of tools the model is allowed to call — is [Dynamic tool list](../dynamic-tool-list/README.md) (`buildToolList()`). This PRD owns invocation only.
...
- **Proving a tool does not exist anywhere.** A syntactically valid name pointing at a non-existent tool lands as `tool-error`, not `unknown-tool` — the router cannot distinguish "never existed" from "currently down" without a registry lookup. Non-existence detection belongs to [Dynamic tool list](../dynamic-tool-list/README.md).
