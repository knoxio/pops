...
- [ ] `ai_inference_daily` Drizzle schema exists with columns: `id` (serial PK), `date` (date, NOT NULL), `provider` (text, NOT NULL), `model` (text, NOT NULL), `operation` (text, NOT NULL), `domain` (text, nullable), `total_calls` (integer, NOT NULL), `total_input_tokens` (bigint, NOT NULL), `total_output_tokens` (bigint, NOT NULL), `total_cost_usd` (numeric, NOT NULL), `avg_latency_ms` (integer, NOT NULL), `error_count` (integer, NOT NULL), `timeout_count` (integer, NOT NULL), `cache_hit_count` (integer, NOT NULL), `budget_blocked_count` (integer, NOT NULL)
...
- [ ] The aggregation and deletion run inside a transaction to prevent data loss if the job fails partway through
...
- Use `bigint` for token columns in the daily table since aggregated totals over many days can exceed 32-bit integer range.
...
- The transaction wrapping aggregation + deletion is critical: if deletion succeeds but aggregation fails (or vice versa), data would be lost or double-counted.
