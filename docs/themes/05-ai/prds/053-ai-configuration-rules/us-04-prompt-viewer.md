# US-04: Prompt template viewer

> PRD: [053 — AI Configuration & Rules](README.md)
> Status: Not started

## Description

As a user, I want to see the prompts the AI uses so that I understand what it's doing with my data.

## Acceptance Criteria

- [ ] Read-only display of current prompt templates (entity matching, categorisation)
- [ ] Syntax-highlighted or code-block formatted
- [ ] Shows which model each prompt is used with
- [ ] No editing capability — prompts live in code

## Notes

This is transparency, not configuration. Users can see exactly what data is sent to Claude and what instructions it receives.
