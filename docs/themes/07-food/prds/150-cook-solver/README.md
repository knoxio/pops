...
export type SolveRecipeRow = {
  ...
  subsNeeded: number;                               // 0 = clean; positive = subs required
...
**Performance**: For 500 recipes × 12 lines × ~5 candidate subs per line, this is ~30k row reads. SQLite handles in <100ms with the existing indexes (`idx_recipe_lines_version`, `idx_batches_variant_prep`, `idx_subs_from_*`). Profile with a seeded fixture; cache deferred to v2.

### Shared substitution-query service

PRD-149's `food.substitutions.resolveForLine` and PRD-150's solver both call the same underlying logic. Per the Epic 06 build order, the service lives in:

```
apps/pops-api/src/modules/food/services/substitutions-resolve.ts
```

Both PRDs' routers wrap this service. If PRD-150 ships first, the service is created here; if PRD-149 ships first, the service lives there. Either way, the contract is: `(recipeVersionId, lineIndex) → { line + candidates + batches }`.

## Sidebar integration

PRD-118's `app-food` manifest gains a new sub-nav entry: "Solve" → `/food/solve`. After existing Food entries (Recipes / Plan / Fridge), order = 60 (incrementally after Fridge's order).
...
- Filters compose (AND across filter groups; OR within a group). Tags filter is AND (recipe must have all selected tags).
- Polling: `food.solver.canICook` refetches every 60s while the page is visible (batches change as the user cooks).
...
| 500-recipe library                                                                                         | <100ms server-side per the perf notes. Acceptable.                                                                                  |
| Recipe was just cooked; `last_cooked_at` is now                                                            | Sort moves it to the bottom of its `subsNeeded` band (DESC nulls-last = recent times are first, oldest last; wait this is reversed) |
...
| Recipe with 0 ingredient lines (compiled but empty)                                                        | `subsNeeded=0`, cookable=true. Edge of the schema; user can still cook it.                                                          |
...
- [ ] Polling refetches every 60s while page visible.
...
- [ ] Optional lines never block cookability.
