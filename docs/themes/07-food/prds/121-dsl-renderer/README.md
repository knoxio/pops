...
export type RecipeVersionWithCompiledData = {
  ...
  lines: RecipeLineWithResolved[]; // from PRD-116; joined with ingredients/variants/prep_states for display
...
The parent page assembles `RecipeVersionWithCompiledData` via a single tRPC procedure (`food.recipes.getForRendering(versionId)`) that does the joins server-side. Component is pure presentation.
...
- Thumbnail variant for `variant='compact'` uses a smaller path (`hero-thumb.jpg`) generated by PRD-124's upload pipeline. Fallback to the full image if the thumbnail doesn't exist.
