...
- The substitution graph answers "out of butter, what works in this savoury recipe?" for at least one tested recipe per cuisine tag
...
| Doc protocol           | PRDs carry acceptance criteria inline; no per-PRD user stories                                          | Theme-local exception to `docs/CLAUDE.md` Ticket Rule; PRDs are split more granularly to compensate                                                    |
