...
- [ ] A `PlexusAdapter` TypeScript interface is exported from a shared module with five required methods: `initialize(config: AdapterConfig): Promise<void>`, `ingest(options: IngestOptions): Promise<EngineData[]>`, `emit(options: EmitOptions, content: EmitContent): Promise<void>`, `healthCheck(): Promise<AdapterStatus>`, `shutdown(): Promise<void>`
...
- The `BaseAdapter` abstract class reduces boilerplate for simple adapters — most adapters will override `ingest` and `healthCheck` at minimum.
