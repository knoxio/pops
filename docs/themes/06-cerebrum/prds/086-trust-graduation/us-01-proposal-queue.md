...
- [ ] A React component in pops-shell displays pending Glia proposals as a reviewable list, showing: action type (prune/consolidate/link/audit), affected engram titles, rationale summary, and creation timestamp (deferred — current page renders id+rationale only, expansion + affected titles tracked in #2578)
- [ ] Each proposal card expands to show the full payload: for prune — staleness score breakdown; for consolidate — proposed merged content diff; for link — the two engrams and relationship reason; for audit — contradiction summary or quality score with suggestions (deferred — tracked in #2578)
- [ ] Three action buttons per proposal: Approve (executes the action), Reject (marks as rejected, no execution), Modify (opens an inline editor for the payload, then approves the modified version) (deferred — Modify currently sends `decision: modify` with no payload editor; tracked in #2578)
...
- [ ] Moltbot notifications are sent for new proposals when the user has enabled Glia notifications — the notification includes a summary and quick-action buttons for approve/reject (modify requires the shell UI) (deferred — tracked in #2578)
- [ ] The review queue shows a badge count of pending proposals in the shell navigation (deferred — tracked in #2578)
