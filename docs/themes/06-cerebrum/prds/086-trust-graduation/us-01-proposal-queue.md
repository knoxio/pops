...
- [ ] Proposals are filterable by action type and sortable by creation date (newest first by default)
- [ ] When a proposal is approved, the corresponding Glia action is executed via `cerebrum.glia.proposals.decide` and the proposal is removed from the queue with a brief success indicator
- [ ] When a proposal is rejected, the user can optionally provide a rejection note that is stored in the `user_note` field
