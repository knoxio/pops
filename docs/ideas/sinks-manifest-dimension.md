...
Follow-on to the shipped [sinks PRD](../themes/13-pillar-finale/prds/sinks-manifest-dimension/README.md). The schema field, orchestrator `publishEvent` dispatcher, and server `createSinkHandler` helper are built and tested. The items below are not.
