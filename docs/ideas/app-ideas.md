...
- **Finance** — Budgeting, transaction tracking, wishlist, bank imports. Heavily automated. _Already exists. Retroactive product spec complete — see [finance spec](../../pillars/finance/docs/README.md)._
- **Home Inventory** — Physical connectivity graph, hierarchical locations, photos, asset IDs, Paperless-ngx receipt linking, warranty tracking, insurance reports. Highest daily-use app. _Partially exists. Fully specced — see [inventory theme](../../pillars/inventory/docs/README.md)._
...
- **Media** — Preference learning and recommendation engine for movies and TV shows. 1v1 pairwise comparisons (ELO-scored) build a taste profile across configurable dimensions. TMDB (movies), TheTVDB (TV), Plex sync, Radarr/Sonarr status. _Fully specced — see [media theme](../../pillars/media/docs/README.md)._
- **Travel Planner** — Trip planning, organising, tracking. Links to finance (trip budgets), recipes (local cuisine).
- **AI Hub** — Integrated AI agents that can query and act across all apps. Cross-domain intelligence layer. Universal object URIs ([ADR-012](../architecture/adr-012-universal-object-uri.md)) enable the AI to reference any object in the system.
...
- ~~**Media Tracker** (original framing)~~ — Evolved from "tracker" to "preference learning and recommendation engine." The tracking is the input, recommendations are the output. See [media theme](../../pillars/media/docs/README.md).
...
| AI Hub      | Everything | Natural language queries across all domains via [universal URIs](../architecture/adr-012-universal-object-uri.md) |
