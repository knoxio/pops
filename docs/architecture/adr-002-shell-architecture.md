...
The set of installed modules is a runtime decision driven by the `POPS_APPS` and `POPS_OVERLAYS` env vars; the shell composes routers and routes from the manifests of installed modules only. See [Epic: Modular Module Runtime](../themes/foundation/epics/modular-module-runtime.md) and [plugin-contract (ModuleManifest + Tier 1 loader)](../themes/foundation/prds/plugin-contract/README.md).
