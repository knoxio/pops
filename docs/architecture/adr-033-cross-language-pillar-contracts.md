...
1. POPS already emits an OpenAPI snapshot per pillar (PRD-195, all 7 pillars). The codegen pipeline exists.
...
The OpenAPI snapshot is the canonical cross-language pillar contract. Every pillar publishes `openapi/<pillar>.openapi.json` as part of its contract package (already true per PRD-195). Cross-language consumers and producers use language-appropriate OpenAPI codegen against that snapshot.
