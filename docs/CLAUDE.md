...
| 3   | **Theme README PRD table**                      | Update the PRD's status in the theme's `## PRD Index` table (and the epic's row in `## Epics`, if the theme uses epics). A theme is `Done` when all its PRDs are done.                                                                                              |
