...
- [ ] `inventory.items.list` — paginated (limit/offset), filterable by search (name LIKE), type, locationId (with optional includeChildren for subtree), and condition; ordered by name ASC
- [ ] `inventory.items.list` with includeChildren=true returns items at the specified location and all descendant locations
- [ ] `inventory.items.get` — returns single item by id with location breadcrumb array, connection count, and photo count; 404 if not found
- [ ] `inventory.items.create` — requires name and type, validates assetId uniqueness if provided, sets createdAt/updatedAt, returns created item
- [ ] `inventory.items.update` — partial update by id, updates only provided fields plus updatedAt; validates assetId uniqueness if changed
- [ ] `inventory.items.delete` — removes item by id; FK cascade deletes connections and photos; 404 if not found
- [ ] `inventory.items.searchByAssetId` — exact match, case-insensitive; returns item or null
- [ ] `inventory.locations.getTree` — returns full location tree as nested nodes with item count per location
- [ ] `inventory.locations.getPath` — returns breadcrumb array from root to specified location (ordered root-first)
- [ ] `inventory.locations.create` — requires name; parentId is optional (null = root location)
- [ ] `inventory.locations.update` — partial update: rename, move (change parentId), or reorder (change sortOrder)
- [ ] `inventory.locations.delete` — cascades to child locations; items at deleted locations get locationId=NULL; force=true skips confirmation check
