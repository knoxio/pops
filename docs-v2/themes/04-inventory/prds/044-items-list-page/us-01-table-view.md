...
- [ ] Columns are sortable: Name (A-Z/Z-A), Type (A-Z/Z-A), Replacement Value (high/low), Purchase Date (newest/oldest)
- [ ] Default sort is Name ASC
- [ ] Clicking a column header toggles sort direction; active sort column is visually indicated
- [ ] Clicking a row navigates to `/inventory/items/:id`
...
- [ ] Asset ID column shows the ID or a dash if null
- [ ] Replacement Value column formats as currency; shows dash if null
- [ ] Purchase Date column formats as locale date; shows dash if null
- [ ] Pagination controls below the table with page size selector and page navigation
- [ ] Table calls `inventory.items.list` with current sort, filters, page, and page size
