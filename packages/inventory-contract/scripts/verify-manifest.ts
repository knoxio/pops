...
try {
  ...
  console.error(
    `[inventory-contract] ${MANIFEST_OUTPUT_PATH} is missing. Run \`pnpm -F @pops/inventory-contract generate:manifest\` and commit the result.`
  );
  process.exit(1);
}
...
if (actual !== expected) {
  console.error(
    `[inventory-contract] ${MANIFEST_OUTPUT_PATH} is out of date. Run \`pnpm -F @pops/inventory-contract generate:manifest\` and commit the result.`
  );
  process.exit(1);
}

process.stdout.write(
  `[inventory-contract] manifest.generated.ts is up to date (version=${version})\n`
);
