/**
 * OpenAPI generator for `@pops/cerebrum-contract` (Theme 13 PRD-153 US-04).
 *
 * Why this script is hand-rolled rather than using `trpc-to-openapi`:
 *
 * The cerebrum pillar's tRPC router (in `@pops/cerebrum-api`) is
 * intentionally tRPC-only — adding `.meta({ openapi: { ... } })` to every
 * procedure would be a separate, invasive change touching every router
 * file in cerebrum-api and would require wiring `OpenApiMeta` into the
 ...
 * `cerebrumRouter` here would pull the entire cerebrum-api runtime graph
 * (`@pops/cerebrum-db`, drizzle, …) into the contract's build step, which
 * defeats the "contract has no runtime dependencies on pillar packages"
 * rule from PRD-153.
 *
 * Output:
 *   - OpenAPI 3.x JSON written to `openapi/cerebrum.openapi.json`
...
function buildDocument(): OpenApiDocument {
  return {
    openapi: '3.0.3',
    info: {
      title: '@pops/cerebrum-contract',
      description:
        "OpenAPI snapshot of the cerebrum pillar's public wire surface. " +
...
function main(): void {
  ...
  const outFile = resolve(HERE, '..', 'openapi', 'cerebrum.openapi.json');
  mkdirSync(dirname(outFile), { recursive: true });
  writeFileSync(outFile, serialized, 'utf8');
  process.stdout.write(`[cerebrum-contract] wrote OpenAPI snapshot to ${outFile}\n`);
}
