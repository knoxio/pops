/**
 * OpenAPI generator for `@pops/core-contract` (Theme 13 PRD-153 US-04).
 *
 * Why this script is hand-rolled rather than using `trpc-to-openapi`:
 *
 * The core pillar's tRPC router (in `@pops/core-api`) is intentionally
 * tRPC-only — adding `.meta({ openapi: { ... } })` to every procedure
 * would be a separate, invasive change touching every router file in
 * core-api and would require wiring `OpenApiMeta` into the trpc builder.
 * That is option (a) from PRD-153's investigation guidance and out of
 * scope here.
 *
 ...
 * `coreRouter` here would pull the entire core-api runtime graph
 * (`@pops/core-db`, etc.) into the contract's build step, which defeats
 * the "contract has no runtime dependencies on pillar packages" rule
 * from PRD-153.
 *
 * Output:
 *   - OpenAPI 3.x JSON written to `openapi/core.openapi.json`
...
function buildDocument(): OpenApiDocument {
  return {
    openapi: '3.0.3',
    info: {
      title: '@pops/core-contract',
      description:
        "OpenAPI snapshot of the core pillar's public wire surface. " +
...
function main(): void {
  ...
  const outFile = resolve(HERE, '..', 'openapi', 'core.openapi.json');
  mkdirSync(dirname(outFile), { recursive: true });
  writeFileSync(outFile, serialized, 'utf8');
  process.stdout.write(`[core-contract] wrote OpenAPI snapshot to ${outFile}\n`);
}
