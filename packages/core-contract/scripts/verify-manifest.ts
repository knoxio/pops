...
try {
  ...
  console.error(
    `[core-contract] ${MANIFEST_OUTPUT_PATH} is missing. Run \`pnpm -F @pops/core-contract generate:manifest\` and commit the result.`
  );
  process.exit(1);
}
...
if (actual !== expected) {
  console.error(
    `[core-contract] ${MANIFEST_OUTPUT_PATH} is out of date. Run \`pnpm -F @pops/core-contract generate:manifest\` and commit the result.`
  );
  process.exit(1);
}

process.stdout.write(`[core-contract] manifest.generated.ts is up to date (version=${version})\n`);
