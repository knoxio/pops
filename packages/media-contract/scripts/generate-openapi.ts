/**
 * OpenAPI generator for `@pops/media-contract` (Theme 13 PRD-153 US-04).
 *
 * Why this script is hand-rolled rather than using `trpc-to-openapi`:
 *
 * The media pillar's tRPC router (in `@pops/media-api`) is intentionally
 * tRPC-only — adding `.meta({ openapi: { ... } })` to every procedure
 * would be a separate, invasive change touching every router file in
 * media-api and would require wiring `OpenApiMeta` into the trpc
 * builder. That is option (a) from PRD-153's investigation guidance and
 * out of scope here.
 *
 ...
 * `mediaRouter` here would pull the entire media-api runtime graph
 * (`@pops/media-db`, drizzle, …) into the contract's build step, which
 * defeats the "contract has no runtime dependencies on pillar packages"
 * rule from PRD-153.
 *
 * Output:
 *   - OpenAPI 3.x JSON written to `openapi/media.openapi.json`
...
function buildDocument(): OpenApiDocument {
  return {
    openapi: '3.0.3',
    info: {
      title: '@pops/media-contract',
      description:
        "OpenAPI snapshot of the media pillar's public wire surface. " +
        "Derived from the contract's Zod schemas. Consumed by iOS Swift " +
        'codegen.',
      version: CONTRACT_VERSION,
    },
    servers: [{ url: '/api/v1', description: 'Media pillar API' }],
    tags: [{ name: 'movies', description: 'Movies' }],
    paths: buildPaths(),
...
function main(): void {
  ...
  const outFile = resolve(HERE, '..', 'openapi', 'media.openapi.json');
  mkdirSync(dirname(outFile), { recursive: true });
  writeFileSync(outFile, serialized, 'utf8');
  process.stdout.write(`[media-contract] wrote OpenAPI snapshot to ${outFile}\n`);
}
