...
import type { OwnedAdapter } from '../../search-adapters.js';
...
function own(moduleId: string, adapter: SearchAdapter): OwnedAdapter {
  return { moduleId, adapter };
}

describe('searchAll', () => {
  it('returns empty sections when no adapters provided', async () => {
    const result = await searchAll({ text: 'test' }, defaultContext, { adapters: [] });
    expect(result.sections).toEqual([]);
  });

  it('fans query to all provided adapters', async () => {
    ...
    await searchAll(query, defaultContext, {
      adapters: [own('media', adapter1), own('media', adapter2)],
    });
  ...
  it('returns sections for each adapter with results', async () => {
    const result = await searchAll({ text: 'test' }, defaultContext, {
      adapters: [
        own('media', makeAdapter('movies', [makeHit()], { icon: 'Film', color: 'purple' })),
        own('media', makeAdapter('tv-shows', [makeHit()], { icon: 'Tv', color: 'purple' })),
      ],
    });
  ...
  it('omits sections with no results', async () => {
    const result = await searchAll({ text: 'test' }, defaultContext, {
      adapters: [
        own('media', makeAdapter('movies', [makeHit()])),
        own('media', makeAdapter('tv-shows', [])),
      ],
    });
  ...
  it('includes correct section metadata', async () => {
    ...
    expect(section.moduleId).toBe('media');
  ...
  describe('failure isolation', () => {
    it('omits failed adapter and returns others', async () => {
      ...
      const result = await searchAll({ text: 'test' }, defaultContext, {
        adapters: [
          own('media', makeFailingAdapter('movies')),
          own('media', makeAdapter('tv-shows', [makeHit()])),
        ],
      });
      warnSpy.mockRestore();
    ...
    it('logs warning for failed adapter', async () => {
      ...
      await searchAll({ text: 'test' }, defaultContext, {
        adapters: [own('media', makeFailingAdapter('movies'))],
      });
  ...
  describe('context ordering', () => {
    it('marks context sections with isContextSection=true', async () => {
      const result = await searchAll(
        { text: 'test' },
        { app: 'media', page: 'search' },
        {
          adapters: [
            own('media', makeAdapter('movies', [makeHit()])),
            own('finance', makeAdapter('transactions', [makeHit()])),
          ],
        }
      );
    ...
    it('sorts context sections before non-context sections', async () => {
      const result = await searchAll(
        ...
        {
          adapters: [
            own('finance', makeAdapter('transactions', [makeHit({ score: 1.0 })])),
            own('media', makeAdapter('movies', [makeHit({ score: 0.5 })])),
          ],
        }
      );
      expect(result.sections[0]!.domain).toBe('movies');
    ...
    it('sorts non-context sections by highest score descending', async () => {
      const result = await searchAll(
        ...
        {
          adapters: [
            own('finance', makeAdapter('transactions', [makeHit({ score: 0.5 })])),
            own('finance', makeAdapter('budgets', [makeHit({ score: 0.8 })])),
          ],
        }
      );
    ...
    it('handles null app context gracefully', async () => {
      const result = await searchAll(
        { text: 'test' },
        { app: null, page: null },
        { adapters: [own('media', makeAdapter('movies', [makeHit()]))] }
      );
  ...
  describe('section limiting', () => {
    it('limits hits to 5 per section', async () => {
      ...
      const result = await searchAll({ text: 'test' }, defaultContext, {
        adapters: [own('media', makeAdapter('movies', hits))],
      });
...
describe('showMore', () => {
  ...
  it('only queries the specified domain adapter', async () => {
    ...
    await showMore({
      ...
      context: defaultContext,
      offset: 0,
      adapters: [own('media', moviesAdapter), own('media', tvAdapter)],
    });
