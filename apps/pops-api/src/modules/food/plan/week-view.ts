...
import type { WeekView, WirePlanEntryRow, WirePlanSlotRow } from '@pops/app-food-db';
...
export function listWireSlots(db: FoodDb): readonly WirePlanSlotRow[] {
  const rows = db
    .select()
    .from(planSlots)
...
function listWireEntries(
  db: FoodDb,
  weekStart: string,
  weekEnd: string
): readonly WirePlanEntryRow[] {
