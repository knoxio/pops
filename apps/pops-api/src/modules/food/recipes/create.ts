...
import { mapCreateRecipeError, MissingRecipeHeaderError } from './error-mapping.js';
...
function parseOrReject(dsl: string): { header: RecipeHeader; ast: RecipeAst } {
  ...
  throw new TRPCError({
    code: 'BAD_REQUEST',
    message: first?.message ?? 'DSL parse failed',
    cause: first?.code === 'MissingRecipeHeader' ? new MissingRecipeHeaderError() : undefined,
  });
}
...
function sourceVersionFor(
  ...
  if (currentVersionId === null) {
    throw new TRPCError({
      code: 'BAD_REQUEST',
      message: `Recipe "${slug}" has no current version to fork from. Open the existing draft instead.`,
    });
  }
  const v = db
    .select()
  ...
  if (v === undefined) {
    throw new TRPCError({
      ...
      message: 'recipe.current_version_id points at a missing row',
    });
  }
  return {
    title: v.title,
