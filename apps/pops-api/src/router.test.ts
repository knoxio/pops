...
function routerKeys(): string[] {
  // `_def.procedures` is the tRPC runtime mapping of router/procedure key
  // to definition. The keys are exactly the top-level router ids we
  // composed; sort for stable comparison.
  return Object.keys(appRouter._def.procedures).toSorted();
}
