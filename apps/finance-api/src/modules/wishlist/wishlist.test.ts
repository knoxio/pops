...
describe("wishlist.list", () => {
  ...
  it("filters by search (case-insensitive LIKE on item)", async () => {
    ...
    const result = await caller.wishlist.list({ search: "mac" });
  ...
  it("filters by priority", async () => {
    ...
    const result = await caller.wishlist.list({ priority: "Dreaming" });
    expect(result.data).toHaveLength(1);
    expect(result.data[0].item).toBe("AirPods Max");
  });
  ...
  it("combines search and priority filters", async () => {
    ...
    const result = await caller.wishlist.list({ search: "macbook", priority: "Needing" });
    expect(result.data).toHaveLength(1);
    expect(result.data[0].item).toBe("MacBook Pro");
  });
  ...
  it("paginates with limit and offset", async () => {
    ...
    const page1 = await caller.wishlist.list({ limit: 3, offset: 0 });
    ...
    const page2 = await caller.wishlist.list({ limit: 3, offset: 3 });
  ...
  it("throws UNAUTHORIZED without auth", async () => {
    ...
    await expect(unauthCaller.wishlist.list({})).rejects.toThrow(TRPCError);
    await expect(unauthCaller.wishlist.list({})).rejects.toMatchObject({
      code: "UNAUTHORIZED",
    });
  });
});
...
describe("wishlist.get", () => {
  ...
  it("throws NOT_FOUND for non-existent ID", async () => {
    await expect(caller.wishlist.get({ id: "does-not-exist" })).rejects.toThrow(TRPCError);
    await expect(caller.wishlist.get({ id: "does-not-exist" })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  });
});
...
describe("wishlist.create", () => {
  it("creates an item with required fields only", async () => {
    const result = await caller.wishlist.create({ item: "MacBook Pro" });
  ...
  it("rejects missing item field", async () => {
    ...
    await expect(caller.wishlist.create({})).rejects.toThrow(TRPCError);
    // @ts-expect-error - Testing validation with missing required field
    await expect(caller.wishlist.create({})).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });

  it("rejects empty item field", async () => {
    await expect(caller.wishlist.create({ item: "" })).rejects.toThrow(TRPCError);
    await expect(caller.wishlist.create({ item: "" })).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });
  ...
  it("rejects invalid priority value", async () => {
    await expect(
      ...
      caller.wishlist.create({ item: "Test", priority: "Invalid" })
  ...
  it("rejects invalid URL", async () => {
    await expect(caller.wishlist.create({ item: "Test", url: "not-a-url" })).rejects.toThrow(
      TRPCError
    );
    await expect(caller.wishlist.create({ item: "Test", url: "not-a-url" })).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });

  it("persists to the database", async () => {
    await caller.wishlist.create({ item: "New Item" });
...
describe("wishlist.update", () => {
  it("updates a single field", async () => {
    ...
    const result = await caller.wishlist.update({ id, data: { priority: "Needing" } });
  ...
  it("clears a field by setting to null", async () => {
    ...
    const result = await caller.wishlist.update({ id, data: { priority: null } });
  ...
  it("rejects empty item", async () => {
    ...
    await expect(caller.wishlist.update({ id, data: { item: "" } })).rejects.toThrow(TRPCError);
    await expect(caller.wishlist.update({ id, data: { item: "" } })).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
  });
  ...
  it("rejects invalid priority on update", async () => {
    ...
    await expect(
      // @ts-expect-error - Testing validation with invalid priority
      caller.wishlist.update({ id, data: { priority: "Invalid" } })
    ).rejects.toThrow(TRPCError);
    await expect(
      // @ts-expect-error - Testing validation with invalid priority
      caller.wishlist.update({ id, data: { priority: "Invalid" } })
...
describe("wishlist.delete", () => {
  it("deletes an existing item", async () => {
    ...
    const result = await caller.wishlist.delete({ id });
  ...
  it("throws NOT_FOUND for non-existent ID", async () => {
    await expect(caller.wishlist.delete({ id: "does-not-exist" })).rejects.toThrow(TRPCError);
    await expect(caller.wishlist.delete({ id: "does-not-exist" })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  });
  ...
  it("is idempotent — second delete throws NOT_FOUND", async () => {
    ...
    await caller.wishlist.delete({ id });
    await expect(caller.wishlist.delete({ id })).rejects.toThrow(TRPCError);
    await expect(caller.wishlist.delete({ id })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  });
});
