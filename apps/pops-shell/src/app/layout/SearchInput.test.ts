...
describe("showMore offset calculation", () => {
  ...
  it("resets extra hits to empty object on query change", () => {
    // Verify reset produces empty object (simulates useEffect on query change)
    const previousHits: Record<string, unknown[]> = { movies: [{ uri: "a" }] };
    expect(previousHits).toHaveProperty("movies");
    const extraHits: Record<string, unknown[]> = {};
